...
def get_data():
    ...
    c = conn.cursor()
    c_output = c.execute("select * from board_online_usr_log where log_time > datetime(\'now\',\'localtime\',\'-15 minute\');")
    return c_output
...
def get_board_data(board_name):
    conn = sqlite3.connect('db/ptt_watcher.db')
    c = conn.cursor()
    c_output = c.execute("select log_time, [" + board_name.replace('-','_') + "] from board_online_usr_log where log_time > datetime(\'now\',\'localtime\',\'-15 minute\');")
    return c_output
...
app.layout = html.Div([
    html.Div(children=[
        dcc.Graph(id='graph-content1', style={'width': '700px','height': '600px','display': 'inline-block'}),
        dcc.Interval(
        ...
        html.Div(children=[
            dcc.Graph(id='graph-content2', style={'width': '600px','height': '300px'}),
            dcc.Interval(
                id='interval-component2',
                interval=4000, # in milliseconds
                n_intervals=0
            ),
            dcc.Graph(id='graph-content3', style={'width': '600px','height': '300px'}),
            dcc.Interval(
    ...
    html.Div(children=[
        # html.H2(children='監控ALL', style={'textAlign':'start'}),
        dcc.Graph(id='graph-content', style={'width': '1370px','height': '650px','display': 'inline-block'}),
        dcc.Interval(
...
def update_graph(n):
    ...
    column_data = chart_data.tail(1).squeeze(axis=0).drop('log_time').sort_values(ascending=False).head(10).index.to_list()
    return px.line(chart_data, x='log_time', y=column_data)
